...
export const config: Config = {
    ...
    token: process.env.TOKEN ? process.env.TOKEN : 'NAO_DEFINIDO',
    // Pterodactyl api token
    pteroToken: process.env.PTERO_TOKEN ?? 'NAO_DEFINIDO',
    // Pterodactyl client api token
    pteroClientToken: process.env.PTERO_CLIENT_TOKEN ?? 'NAO_DEFINIDO',
    // Pterodactyl HOST address
    pteroHost: process.env.PTERO_HOST ? process.env.PTERO_HOST : 'NAO_DEFINIDO',
    ...
    permLevels: [
        ...
        },
        {
            ...
            // Do not change !
            check: (message) => '580416011472338957' === message.author.id,
        },
    ],
};
